...
public class StudentProfileService {
    ...
    public List<StudentProfileOptionDto> getOptions(String username, String query) {
        log.info("Loading student profile options for username='{}', query='{}'", username, query);
        ...
        if ("student".equals(context.roleCode())) {
            List<StudentProfileOptionDto> result = jdbcTemplate.query(
                    """
                    select s.student_id,
                           ua.full_name,
                           sg.group_code,
            ...
            log.info(
                    "Student profile options loaded for username='{}', role='student', count={}",
                    username,
                    result.size()
            );
            return result;
        ...
        log.info(
                "Student profile options loaded for username='{}', role='{}', count={}",
                username,
                context.roleCode(),
                result.size()
        );
    ...
    public StudentProfileResponseDto getProfile(String username, Integer studentId) {
        log.info("Loading student profile for username='{}', requestedStudentId={}", username, studentId);
        ...
        if (headers.isEmpty()) {
            log.warn("Student profile not found for studentId={}", allowedStudentId);
        ...
        log.info(
                "Student profile loaded successfully for studentId={}, courses={}",
                allowedStudentId,
                subjects.size()
        );
    ...
    private StudentProfileCourseDto mapCourse(CourseRow row) {
        log.debug(
                "Mapping course data for enrollmentId={}, courseId={}, courseCode='{}'",
                row.enrollmentId(),
                row.courseId(),
                row.courseCode()
        );
        ...
        log.debug(
                "Course data mapped for enrollmentId={}, gradesCount={}, recommendationsCount={}",
                row.enrollmentId(),
                grades.size(),
    ...
    private AccessContext resolveAccessContext(String username) {
        ...
        log.debug(
                "Access context resolved for username='{}', role='{}', studentId={}",
                username,
                roleCode,
                studentId
        );
    ...
    private Integer ensureStudentAccess(AccessContext context, Integer studentId) {
        if ("student".equals(context.roleCode())) {
            if (context.studentId() == null) {
                log.warn("Student access check failed: student profile is not filled");
