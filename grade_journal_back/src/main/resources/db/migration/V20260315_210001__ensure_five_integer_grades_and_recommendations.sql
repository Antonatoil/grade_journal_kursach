...
-- ---------------------------------------------------------
-- 1. Все старые оценки переводим в целые
-- ---------------------------------------------------------
update grade
set grade_value = round(grade_value, 0);

-- ---------------------------------------------------------
-- 2. Удаляем прогнозы и рекомендации только для текущего семестра
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 3. Берем первые 5 уже прошедших занятий по каждой дисциплине
-- ---------------------------------------------------------
with current_term as (
    select term_id
    from academic_term
...
select
    ...
    null
from enrollment e
join first_five_lessons ffl
    on ffl.offering_id = e.offering_id
on conflict (enrollment_id, schedule_id)
do update set is_present = true;

-- ---------------------------------------------------------
-- 4. Создаем assessment_item на первых 5 занятиях, если их нет
-- ---------------------------------------------------------
with current_term as (
    select term_id
    from academic_term
...
select
    ffl.offering_id,
    case
        when ffl.rn in (2, 4) then (select assessment_type_id from lab_type)
        else (select assessment_type_id from quiz_type)
    end as assessment_type_id,
    case
        when ffl.rn = 1 then 'Оценивание за занятие 1'
        when ffl.rn = 2 then 'Оценивание за занятие 2'
        when ffl.rn = 3 then 'Оценивание за занятие 3'
        when ffl.rn = 4 then 'Оценивание за занятие 4'
        else 'Оценивание за занятие 5'
    end as title,
    10.00,
    0.05,
    ffl.lesson_date,
    ffl.schedule_id,
    false
from first_five_lessons ffl
left join assessment_item ai
    on ai.schedule_id = ffl.schedule_id
where ai.assessment_item_id is null;

-- ---------------------------------------------------------
-- 5. Гарантируем 5 оценок на предмет для каждого студента
--    enrollment = студент + конкретный предмет
-- ---------------------------------------------------------
with current_term as (
    select term_id
    from academic_term
...
five_items as (
    ...
    where ai.schedule_id in (select schedule_id from first_five_lessons)
),
target_items as (
    select *
    from five_items
...
select
    ...
    round(
        least(
            ...
            greatest(
                ...
                (
                    5
                    + ((e.enrollment_id + ti.assessment_item_id) % 6)
                    + case
                        when ti.type_code = 'lab' then 1
                        else 0
                      end
                )::numeric
            )
    ...
    case
        when ((e.enrollment_id + ti.assessment_item_id) % 5) = 0
            then 'Нужно внимательнее проработать материал'
        else '—'
    end as teacher_comment,
    now()
from enrollment e
join target_items ti
    on ti.offering_id = e.offering_id
on conflict (enrollment_id, assessment_item_id)
do update set
    grade_value = round(excluded.grade_value, 0),
    teacher_comment = excluded.teacher_comment,
    graded_at = excluded.graded_at;

-- ---------------------------------------------------------
-- 6. Еще раз страхуемся: все оценки только целые
-- ---------------------------------------------------------
update grade
set grade_value = round(grade_value, 0);

-- ---------------------------------------------------------
-- 7. Пересчитываем прогноз по каждому предмету
-- ---------------------------------------------------------
with grade_stats as (
    select
        e.enrollment_id,
...
select
    ...
    'five-integer-grades-with-subject-recommendations-v4',
    now()
from base b;

-- ---------------------------------------------------------
-- 8. Рекомендации по каждому предмету
-- ---------------------------------------------------------
...
select
    ...
    pp.prediction_id,
    'attendance',
    'По дисциплине "' || c.course_name || '" необходимо повысить посещаемость: регулярно присутствовать на занятиях и сократить количество пропусков.',
    case
...
select
    ...
    pp.prediction_id,
    'performance',
    'По дисциплине "' || c.course_name || '" рекомендуется усилить подготовку: повторить теорию, разобрать типовые задания и обратиться к преподавателю за консультацией.',
    case
